    ...
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${isScrolled ? "bg-zinc-950/80 backdrop-blur-md border-b border-white/5" : "bg-transparent"}`}>
      <nav className="mx-auto flex max-w-7xl items-center justify-between p-4 sm:px-6 lg:px-8" aria-label="Global">
        ...
        <div className="hidden lg:flex lg:gap-x-8">
          {navigation.map((item) => (
            <Link key={item.name} href={item.href} className={`text-sm font-medium transition-colors hover:text-white ${pathname === item.href ? "text-white" : "text-slate-400"}`}>
              {item.name}
            </Link>
          ))}
        </div>
        <div className="hidden lg:flex lg:flex-1 lg:justify-end lg:gap-4 items-center">
          ...
          <Link href="/contact" className="rounded-full bg-indigo-500/10 px-4 py-2 text-sm font-semibold text-indigo-400 ring-1 ring-inset ring-indigo-500/20 hover:bg-indigo-500/20 transition-all">Demander une démo</Link>
        </div>
      </nav>
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden glass-panel absolute inset-x-0 top-full p-4 border-b border-white/5 shadow-2xl">
          ...
          <div className="mt-4 border-t border-white/5 pt-4 space-y-2">
            ...
            <Link href="/contact" onClick={() => setIsMobileMenuOpen(false)} className="block rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-indigo-400 bg-indigo-500/10 text-center mt-2">Demander une démo</Link>
          </div>
        </div>
      )}
    </header>
