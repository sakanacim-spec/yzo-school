  ...
  if (student.restant <= 0) {
    return (
      <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-100/50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 rounded-lg text-[10px] font-black uppercase tracking-widest border border-emerald-200/50 dark:border-emerald-500/20">
        <CheckCircle className="w-3.5 h-3.5" /> {t(language as Language, 'documents.responsibleParent') || 'Parent Responsable'}
      </span>
    );
  }
  if (taux >= 0.7) {
    return (
      <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-100/50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 rounded-lg text-[10px] font-black uppercase tracking-widest border border-amber-200/50 dark:border-amber-500/20">
        <span className="font-bold text-amber-500">≥70%</span> {t(language as Language, 'documents.installmentValidated') || 'Tranche Validée'}
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-rose-100/50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 rounded-lg text-[10px] font-black uppercase tracking-widest border border-rose-200/50 dark:border-rose-500/20">
      <AlertTriangle className="w-3.5 h-3.5" /> {t(language as Language, 'documents.pending') || 'En Attente'}
    </span>
  );
};
...
const StudentCard: React.FC<{ student: Student; schoolName: string; schoolYear: string; msgRem: string; msgRap: string; schoolLogo?: string; schoolStamp?: string }> = ({
  ...
  const defaultMsg = student.restant <= 0
    ? (t(language as Language, 'documents.defaultRemMessage') 
        ? (t(language as Language, 'documents.defaultRemMessage') as string).replace('{{name}}', `${student.prenom} ${student.nom}`).replace('{{class}}', student.classe).replace('{{msgRem}}', msgRem).replace('{{school}}', schoolName) 
        : `Bonjour, parent de ${student.prenom} ${student.nom} (${student.classe}). ${msgRem} — ${schoolName}`)
    : (t(language as Language, 'documents.defaultRapMessage')
        ? (t(language as Language, 'documents.defaultRapMessage') as string).replace('{{name}}', `${student.prenom} ${student.nom}`).replace('{{class}}', student.classe).replace('{{restant}}', fmtMoney(student.restant)).replace('{{msgRap}}', msgRap).replace('{{school}}', schoolName)
      ...
      <div className="flex-1">
        ...
        <div className="grid grid-cols-3 gap-3 bg-slate-50 dark:bg-slate-900/50 rounded-2xl p-4 border border-slate-100 dark:border-slate-800/50 mb-5">
            <div className="flex flex-col">
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">{t(language as Language, 'documents.tuition') || 'Écolage'}</span>
            ...
            <div className="flex flex-col border-l border-slate-200 dark:border-slate-700/50 pl-3">
              <span className="text-[9px] font-black text-emerald-500 uppercase tracking-widest mb-1">{t(language as Language, 'documents.paid') || 'Payé'}</span>
            ...
            <div className="flex flex-col border-l border-slate-200 dark:border-slate-700/50 pl-3">
              <span className="text-[9px] font-black text-rose-500 uppercase tracking-widest mb-1">{t(language as Language, 'documents.remaining') || 'Restant'}</span>
              <span className={`font-black text-sm ${student.restant <= 0 ? 'text-emerald-500' : 'text-rose-600 dark:text-rose-400'}`}>
                {student.restant <= 0 ? (t(language as Language, 'documents.ok') || 'OK') : `${new Intl.NumberFormat('fr-FR').format(student.restant)} F`}
              </span>
            </div>
        </div>
        ...
        <div className="mb-2">
            <div className="flex justify-between items-end mb-1.5">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{t(language as Language, 'documents.progress') || 'Progression'}</span>
  ...
  if (students.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] animate-fadeIn">
        ...
        <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight mb-2">{t(language as Language, 'documents.documentCenter') || 'Centre de Documents'}</h2>
        <p className="text-sm font-medium text-slate-500">{t(language as Language, 'documents.importStudentsDesc') || 'Importez des élèves depuis le registre pour générer des documents.'}</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 pb-20 max-w-[1600px] mx-auto animate-slideUp">
      ...
      <div className="relative pro-card p-8 overflow-hidden group bg-white/70 dark:bg-slate-900/70 backdrop-blur-2xl border border-indigo-100 dark:border-indigo-900/30">
        ...
        <div className="relative z-10 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-indigo-500 text-[10px] font-black text-white uppercase tracking-[0.2em] mb-4 shadow-[0_0_15px_rgba(99,102,241,0.4)]">
                <Layers className="w-3.5 h-3.5" /> {t(language as Language, 'documents.editionExports') || 'Édition & Exports'}
            </div>
            <h2 className="text-4xl font-black text-slate-900 dark:text-white tracking-tighter mb-2">
              {t(language as Language, 'documents.centerOf') || 'Centre de'} <span className="text-transparent bg-clip-text bg-gradient-to-br from-indigo-400 to-indigo-600">{t(language as Language, 'documents.documentsHighlight') || 'Documents'}</span>
            </h2>
            <p className="text-slate-600 dark:text-slate-400 text-sm font-medium">
              {t(language as Language, 'documents.docCenterDesc') || 'Générez des rapports par classe, éditez des listes de recouvrement et exportez vos données en Excel.'}
            </p>
        </div>
      </div>
      ...
      <div className="pro-card p-6 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-slate-200/50 dark:border-slate-800">
        <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="font-black text-lg text-slate-900 dark:text-white tracking-tight">{t(language as Language, 'documents.massGeneration') || 'Générations de masse'}</h3>
        </div>
          ...
          {classes.filter((c) => students.some((s) => s.classe === c)).map((classe) => (
            ...
            >
              ...
              <div className="text-center">
                  ...
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{students.filter((s) => s.classe === classe).length} {t(language as Language, 'documents.enrolled') || 'inscrits'}</p>
              </div>
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-4 mt-8 pt-8 border-t border-slate-100 dark:border-slate-800">
          ...
          >
            ...
            {t(language as Language, 'documents.unpaidReports') || 'Rapports Impayés'} ({nonSoldes.length})
          </button>
          <button
          ...
          >
            ...
            {t(language as Language, 'documents.exportDatabase') || 'Exporter Base de Données'}
          </button>
        </div>
        ...
        {generating && (
          <div className="mt-6 flex items-center gap-3 p-4 bg-indigo-50 dark:bg-indigo-500/10 rounded-2xl border border-indigo-100 dark:border-indigo-500/20 animate-pulse">
            ...
            <span className="text-sm font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">{t(language as Language, 'documents.pdfGenerationInProgress') || 'Génération PDF en cours...'}</span>
          </div>
        )}
      </div>
      ...
      <div className="pro-card p-6 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-slate-200/50 dark:border-slate-800">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div className="flex items-center gap-3">
                ...
                <h3 className="font-black text-lg text-slate-900 dark:text-white tracking-tight">{t(language as Language, 'documents.individualReceipts') || 'Reçus Individuels'}</h3>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                <option value="">{t(language as Language, 'documents.allClasses') || 'Toutes les classes'}</option>
              ...
              >
                <option value="">{t(language as Language, 'documents.allStatuses') || 'Tous les statuts'}</option>
                <option value="Soldé">{t(language as Language, 'documents.statusSettled') || 'Soldé'}</option>
                <option value="Partiel">{t(language as Language, 'documents.statusPartial') || 'Partiel'}</option>
                <option value="Non soldé">{t(language as Language, 'documents.statusUnsettled') || 'Non soldé'}</option>
              </select>
              {(filterClasse || filterStatus) && (
                <button 
                  ...
                  title={t(language as Language, 'documents.reset') || 'Réinitialiser'}
                >
                  <X className="w-4 h-4" />
                </button>
              )}
        ...
        <div className="mb-4 px-2 flex justify-between items-center">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{filtered.length} {t(language as Language, 'documents.recordsAvailable') || 'fiches disponibles'}</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-5">
          ...
          {filtered.length === 0 && (
              <div className="col-span-full py-12 text-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl">
                  <p className="text-sm font-bold text-slate-500">{t(language as Language, 'documents.noMatchingRecords') || 'Aucune fiche ne correspond à ces critères.'}</p>
              </div>
          )}
        </div>
      </div>
