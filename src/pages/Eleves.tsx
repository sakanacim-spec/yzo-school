              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-2xl font-black text-slate-800 tracking-tight">
                    {student ? (t(language as Language, 'students.editStudent') || 'Modifier l\'élève') : (t(language as Language, 'students.newRegistration') || 'Nouvelle Inscription')}
                  </h3>
                  <p className="text-sm font-bold text-slate-400 mt-1">
                    {t(language as Language, 'common.step') || 'Étape'} {modalStep} {t(language as Language, 'common.of') || 'sur'} 2 : {modalStep === 1 ? (t(language as Language, 'students.identity') || 'Identité') : (t(language as Language, 'students.schoolingAndFinance') || 'Scolarité & Finance')}
                  </p>
                </div>
              </div>
              ...
              <form id="student-form" onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className={`space-y-6 transition-all duration-500 ${modalStep === 1 ? 'opacity-100 translate-x-0 block' : 'opacity-0 -translate-x-10 hidden'}`}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                        <User className="w-3 h-3" /> {t(language as Language, 'students.studentLastName') || "Nom de l'élève"} *
                      </label>
                    ...
                    <div className="space-y-2">
                      <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                        <User className="w-3 h-3" /> {t(language as Language, 'students.studentFirstName') || "Prénoms"} *
                      </label>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                        <User className="w-3 h-3" /> {t(language as Language, 'students.gender') || "Sexe"}
                      </label>
                      <div className="relative">
                        <select className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3.5 text-[15px] font-bold text-slate-800 focus:ring-4 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all appearance-none cursor-pointer" value={form.sexe} onChange={(e) => setForm({ ...form, sexe: e.target.value as 'M' | 'F' })}>
                          <option value="M">{t(language as Language, 'students.genderMale') || "Masculin"}</option>
                          <option value="F">{t(language as Language, 'students.genderFemale') || "Féminin"}</option>
                        </select>
                ...
                <div className={`space-y-6 transition-all duration-500 ${modalStep === 2 ? 'opacity-100 translate-x-0 block' : 'opacity-0 translate-x-10 hidden'}`}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        {t(language as Language, 'students.tuitionFeesToPay') || 'Frais de scolarité à payer'} *
                      <div className="relative">
                        <input
                          type="number"
                          min="0"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                        <School className="w-3 h-3" /> {t(language as Language, 'students.previousSchool') || 'École de provenance'}
                      </label>
                  ...
                  {!student && (
                    <div className="bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-100 p-6 rounded-2xl relative overflow-hidden">
                      ...
                      <h4 className="text-[12px] font-black text-emerald-800 uppercase tracking-widest mb-4 flex items-center gap-2 relative z-10">
                        <Wallet className="w-4 h-4" /> {t(language as Language, 'students.financialInfoFirstInstallment') || 'Informations Financières (1er Versement)'}
                      </h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
                        <div className="space-y-2">
                          <label className="text-[11px] font-black text-emerald-700/70 uppercase tracking-widest">{t(language as Language, 'students.amountPaid') || 'Montant payé'} ({currency})</label>
                        ...
                        <div className="space-y-2">
                          <label className="text-[11px] font-black text-emerald-700/70 uppercase tracking-widest">{t(language as Language, 'students.associatedReceiptNo') || 'N° Reçu associé'}</label>
                  ...
                  <label className="flex items-center gap-4 p-4 border border-slate-200 rounded-2xl cursor-pointer hover:bg-slate-50 transition-colors group">
                    ...
                    <span className="text-[13px] font-bold text-slate-700 uppercase tracking-wide">{t(language as Language, 'students.repeatingStudent') || 'Élève redoublant'}</span>
                  </label>
                </div>
              </form>
            </div>
            ...
            <div className="px-8 py-5 border-t border-slate-100 bg-slate-50 flex items-center justify-between">
              {modalStep === 2 ? (
                <button type="button" onClick={() => setModalStep(1)} className="px-6 py-3 text-[13px] font-black uppercase tracking-widest text-slate-500 hover:text-slate-800 transition-colors">
                  {t(language as Language, 'common.back') || 'Retour'}
                </button>
              ) : <div></div>}
...
const WhatsAppBtn: React.FC<{ student: Student; schoolName: string }> = ({ student, schoolName }) => {
  ...
  const defaultMsg = student.restant <= 0
    ? (t(language as Language, 'students.whatsappDefaultSettled') ? (t(language as Language, 'students.whatsappDefaultSettled') as string).replace('{{name}}', `${student.prenom} ${student.nom}`).replace('{{class}}', student.classe).replace('{{tuition}}', formatMontant(student.ecolage, currency)).replace('{{school}}', schoolName) : `Bonjour, parent de ${student.prenom} ${student.nom} (${student.classe}). Nous vous felicitons d'avoir solde la scolarite (${formatMontant(student.ecolage, currency)}). Merci ! — ${schoolName}`)
    : (t(language as Language, 'students.whatsappDefaultUnsettled') ? (t(language as Language, 'students.whatsappDefaultUnsettled') as string).replace('{{name}}', `${student.prenom} ${student.nom}`).replace('{{class}}', student.classe).replace('{{restant}}', formatMontant(student.restant, currency)).replace('{{taux}}', String(taux)).replace('{{school}}', schoolName) : `Bonjour, parent de ${student.prenom} ${student.nom} (${student.classe}). Solde restant : ${formatMontant(student.restant, currency)} (paye : ${taux}%). Merci de regulariser. — ${schoolName}`);
  ...
  return (
    <a
      ...
      title={t(language as Language, 'students.sendOnWhatsapp') || 'Envoyer sur WhatsApp'}
    >
...
const PhotoModal: React.FC<{ student: Student; onClose: () => void; onSave: (b64: string) => Promise<void>; onDelete: () => Promise<void>; }> = ({ student, onClose, onSave, onDelete }) => {
  ...
  const handleDelete = async () => {
    if (confirm(t(language as Language, 'students.confirmDeletePhoto') || 'Voulez-vous vraiment supprimer cette photo ?')) {
        setDeleting(true);
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-fadeIn">
      <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl w-full max-w-sm overflow-hidden border border-slate-200 dark:border-slate-800 animate-slideUp">
        <div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-800">
          <h2 className="text-sm font-black uppercase tracking-widest text-slate-900 dark:text-white">{t(language as Language, 'students.profilePhoto') || 'Photo de profil'}</h2>
        ...
        <div className="p-8 flex flex-col items-center">
          ...
          >
            {preview ? (
              ...
              <div className="flex flex-col items-center text-slate-400 dark:text-slate-500">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-center px-2 group-hover:text-amber-600">{t(language as Language, 'students.takeChoose') || 'Prendre / Choisir'}</span>
              </div>
            )}
            <div className="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center backdrop-blur-sm">
              ...
              <span className="text-white text-[10px] font-black uppercase tracking-widest">{t(language as Language, 'common.edit') || 'Modifier'}</span>
            </div>
          </div>
          ...
          {(student.photoUrl || preview) && (
             <button onClick={handleDelete} disabled={deleting} className="text-rose-500 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-500/10 px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest flex items-center gap-2 mb-4 transition-colors">
                 <Trash2 className="w-3.5 h-3.5" /> {deleting ? (t(language as Language, 'common.deleting') || 'Suppression...') : (t(language as Language, 'common.delete') || 'Supprimer')}
             </button>
          )}

          <div className="flex gap-3 w-full mt-2">
            <button onClick={onClose} className="flex-1 py-3 border border-slate-200 dark:border-slate-700 rounded-xl text-[11px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">{t(language as Language, 'common.cancel') || 'Annuler'}</button>
            <button onClick={handleSave} disabled={saving || !preview || preview === student.photoUrl} className="flex-1 py-3 bg-amber-500 disabled:opacity-50 disabled:hover:scale-100 text-white rounded-xl text-[11px] font-black uppercase tracking-widest hover:bg-amber-600 transition-all hover:scale-105 active:scale-95 flex items-center justify-center gap-2 shadow-lg shadow-amber-500/30">
              {saving ? '...' : (t(language as Language, 'common.validate') || 'Valider')}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6 pb-20 max-w-[1600px] mx-auto animate-slideUp">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div className="max-w-2xl">
            ...
            <p className="text-slate-600 dark:text-slate-400 text-sm font-medium">
              {t(language as Language, 'students.registerDesc') || 'Gérez les inscriptions, consultez les fiches individuelles et suivez les statuts financiers.'}
            </p>
          </div>
          ...
          {(user?.role === 'admin' || user?.role === 'directeur' || user?.role === 'directeur_general' || user?.role === 'comptable') && (
            ...
            >
              ...
              <Plus className="w-4 h-4 text-amber-500" /> {t(language as Language, 'students.newRegistration') || 'Nouvelle Inscription'}
            </button>
          )}
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder={t(language as Language, 'students.searchPlaceholder') || 'Rechercher par nom, prénom, classe ou téléphone...'}
              value={searchQuery}
          ...
          >
            <Filter className="w-4 h-4" /> {t(language as Language, 'students.advancedFilters') || 'Filtres Avancés'}
          </button>
        </div>
        ...
        <div className={`grid transition-all duration-300 ease-in-out overflow-hidden ${showFilters ? 'grid-rows-[1fr] opacity-100 mt-4' : 'grid-rows-[0fr] opacity-0 mt-0'}`}>
          <div className="min-h-0 flex flex-wrap gap-4 pt-1">
            <select className="flex-1 min-w-[200px] bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 rounded-2xl px-5 py-4 text-[13px] font-bold focus:ring-2 focus:ring-amber-500 outline-none transition-all cursor-pointer" value={filterCycle} onChange={(e) => setFilterCycle(e.target.value)}>
              <option value="">{t(language as Language, 'students.allCycles') || 'Tous les cycles'}</option>
            ...
            <select className="flex-1 min-w-[200px] bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 rounded-2xl px-5 py-4 text-[13px] font-bold focus:ring-2 focus:ring-amber-500 outline-none transition-all cursor-pointer" value={filterClasse} onChange={(e) => setFilterClasse(e.target.value)}>
              <option value="Toutes">{t(language as Language, 'students.allClasses') || 'Toutes les classes'}</option>
            ...
            <select className="flex-1 min-w-[200px] bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 rounded-2xl px-5 py-4 text-[13px] font-bold focus:ring-2 focus:ring-amber-500 outline-none transition-all cursor-pointer" value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}>
              <option value="">{t(language as Language, 'students.allFinancialStatuses') || 'Tous les statuts financiers'}</option>
              <option value="Soldé">{t(language as Language, 'students.statusSettled') || 'Soldé'}</option><option value="Partiel">{t(language as Language, 'students.statusPartial') || 'Partiel'}</option><option value="Non soldé">{t(language as Language, 'students.statusUnsettled') || 'Non soldé'}</option>
            </select>
            {(filterClasse || filterCycle || filterStatus) && (
              <button onClick={() => { setFilterClasse(''); setFilterCycle(''); setFilterStatus(''); }} className="flex items-center gap-2 px-6 py-4 text-[12px] font-black uppercase tracking-widest text-rose-500 hover:text-white hover:bg-rose-500 rounded-2xl transition-all border border-rose-200 dark:border-rose-500/30">
                <X className="w-4 h-4" /> {t(language as Language, 'common.reset') || 'Reset'}
              </button>
            )}
          </div>
        </div>
      ...
      <div className="flex items-center justify-between px-2">
        <p className="text-[11px] font-black text-slate-500 uppercase tracking-widest">{filtered.length} {t(language as Language, 'students.records') || 'dossiers'} {t(language as Language, 'common.of') || 'sur'} {students.length}</p>
        {filtered.length > 0 && (
          <p className="text-[11px] font-black text-slate-500 uppercase tracking-widest">
            {t(language as Language, 'students.received') || 'Perçu'} : <strong className="text-emerald-500">{formatMontant(filtered.reduce((a, s) => a + s.dejaPaye, 0), currency)}</strong>
          </p>
        )}
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-slate-800/30 border-b border-slate-100 dark:border-slate-800">
                {[
                  { key: 'nom' as SortKey, label: t(language as Language, 'students.studentIdentity') || 'Identité Élève' },
                  { key: 'classe' as SortKey, label: t(language as Language, 'students.classAndCycle') || 'Classe & Cycle' },
                  { key: null, label: t(language as Language, 'students.parentContact') || 'Contact Parent' },
                  { key: 'dejaPaye' as SortKey, label: t(language as Language, 'students.payments') || 'Paiements' },
                  { key: 'restant' as SortKey, label: t(language as Language, 'students.remaining') || 'Reste' },
                  { key: 'status' as SortKey, label: t(language as Language, 'common.status') || 'Statut' },
                  { key: null, label: t(language as Language, 'common.actions') || 'Actions' },
                ].map((col) => (
                  <th
                    key={col.label}
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-20 text-center">
                    <div className="flex flex-col items-center">
                       ...
                       <p className="text-sm font-black text-slate-500 tracking-tight">{t(language as Language, 'students.noMatchingRecords') || 'Aucun dossier correspondant'}</p>
                    </div>
                  </td>
                </tr>
              ) : (
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        <button
                          ...
                          title={s.photoUrl ? (t(language as Language, 'students.editPhoto') || 'Modifier la photo') : (t(language as Language, 'students.addPhoto') || 'Ajouter une photo')}
                        >
                          {s.photoUrl ? (
                        ...
                        <div>
                          ...
                          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-0.5 flex items-center gap-1">
                            <span className="bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded text-slate-600 dark:text-slate-400">{s.sexe === 'M' ? (t(language as Language, 'students.boy') || 'Garçon') : (t(language as Language, 'students.girl') || 'Fille')}</span>
                            {s.redoublant && <span className="bg-rose-100 dark:bg-rose-500/20 text-rose-600 dark:text-rose-400 px-1.5 py-0.5 rounded">{t(language as Language, 'students.repeater') || 'Redoublant'}</span>}
                          </p>
                        </div>
                      </div>
                    </td>
                      ...
                      <div className="flex items-center gap-2 opacity-50 group-hover/row:opacity-100 transition-opacity">
                        ...
                        {(user?.role === 'admin' || user?.role === 'directeur' || user?.role === 'directeur_general' || user?.role === 'comptable') && (
                          <>
                            ...
                            <button onClick={() => setModal({ open: true, student: s })} className="p-2 hover:bg-amber-100 dark:hover:bg-amber-500/20 rounded-xl text-amber-600 dark:text-amber-400 transition-all hover:scale-110" title={t(language as Language, 'common.edit') || 'Modifier'}>
                              <Edit2 className="w-4 h-4" />
                            </button>
                            <button onClick={() => handleDelete(s.id)} className={`p-2 rounded-xl transition-all hover:scale-110 ${deleteConfirm === s.id ? 'bg-rose-500 text-white shadow-lg shadow-rose-500/30' : 'hover:bg-rose-100 dark:hover:bg-rose-500/20 text-rose-500 dark:text-rose-400'}`} title={deleteConfirm === s.id ? (t(language as Language, 'students.clickToConfirmDeletion') || 'Cliquer pour confirmer la suppression') : (t(language as Language, 'common.delete') || 'Supprimer')}>
        ...
        {filtered.length > 0 && (
          <div className="border-t border-slate-100 dark:border-slate-800 p-5 bg-slate-50/50 dark:bg-slate-900/30 flex flex-wrap gap-6 items-center">
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t(language as Language, 'students.totalExpectedTuition') || 'Écolage Total Attendu'}</span>
            ...
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">{t(language as Language, 'students.totalReceived') || 'Total Perçu'}</span>
            ...
            <div className="w-px h-8 bg-slate-200 dark:bg-slate-700"></div>
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-rose-500 uppercase tracking-widest">{t(language as Language, 'students.remainingToRecover') || 'Reste à recouvrer'}</span>
            ...
            <div className="ml-auto flex items-center gap-4">
              <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-100/50 dark:bg-emerald-500/10 rounded-lg">
                ...
                <span className="text-xs font-bold text-emerald-700 dark:text-emerald-400">{filtered.filter((s) => s.status === 'Soldé').length} {t(language as Language, 'students.settledStatus') || 'Soldés'}</span>
              </div>
              <div className="flex items-center gap-2 px-3 py-1.5 bg-rose-100/50 dark:bg-rose-500/10 rounded-lg">
                <span className="w-2 h-2 rounded-full bg-rose-500"></span>
                <span className="text-xs font-bold text-rose-700 dark:text-rose-400">{filtered.filter((s) => s.status !== 'Soldé').length} {t(language as Language, 'students.pendingStatus') || 'En attente'}</span>
              </div>
            </div>
          </div>
        )}
